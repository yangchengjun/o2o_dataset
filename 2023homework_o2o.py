...
def model_cbc(train, validate):
    ...
    if os.path.exists('output_files/models/cbc.pkl'):
        cbc = joblib.load('output_files/models/cbcb.pkl')
        print('Load cbc model...')
    else:
        print('Train cbc model...')
        cbc = CatBoostClassifier(iterations = 1000, learning_rate = 0.1, depth = 6, loss_function = 'Logloss', eval_metric = 'AUC', logging_level = 'Verbose')
...
def model_lgb(train, validate):
    ...
    X_validate = validate.drop(['User_id', 'Coupon_id', 'Date_received', 'label'], axis=1)
    y_validate = validate['label']
    if os.path.exists('output_files/models/lgb.pkl'):
        lgb = joblib.load('output_files/models/lgb.pkl')
        print('Load LGB model...')
    else:
        print('Train LGB model...')
        lgb = LGBMClassifier()
        lgb.fit(X_train, y_train)
        joblib.dump(lgb, 'output_files/models/lgb.pkl')
    # Predict
    pred = lgb.predict(X_validate)
    # Calculate accuracy
    accuracy = lgb.score(X_validate, y_validate)
    return accuracy
